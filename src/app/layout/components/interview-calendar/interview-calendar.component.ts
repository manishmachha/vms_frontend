...
@Component({
  ...
  template: `
    <div class="calendar-container h-full flex flex-col bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div class="flex-1 overflow-auto p-4">
        ...
        <div class="mt-6 space-y-4" *ngIf="selectedInterviews().length > 0; else noInterviews">
          ...
          <div class="space-y-3">
            <div *ngFor="let interview of selectedInterviews()" 
                 class="group p-3 bg-gray-50/50 rounded-xl border border-gray-100 hover:border-indigo-100 transition-all">
              <div class="flex justify-between items-start mb-1">
                <div>
                  <p class="text-xs font-bold text-gray-900 capitalize">{{ interview.type.toLowerCase().replace('_', ' ') }}</p>
                  <p class="text-[10px] text-gray-500">{{ interview.scheduledAt | date:'shortTime' }} ({{ interview.durationMinutes }}m)</p>
                </div>
                <span class="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100">
                  {{ interview.status }}
                </span>
              </div>
              <div class="flex items-center gap-1.5 text-xs text-gray-600" *ngIf="interview.application?.candidate">
                <mat-icon class="text-[14px] w-[14px] h-[14px] leading-none text-gray-400">person</mat-icon>
                <span class="font-medium">{{ interview.application!.candidate?.firstName }} {{ interview.application!.candidate?.lastName }}</span>
              </div>
            </div>
          </div>
        </div>
...
export class InterviewCalendarComponent implements OnInit {
  ...
  dateClass: MatCalendarCellClassFunction<Date> = (cellDate, view) => {
    if (view === 'month') {
      ...
      const hasInterview = this.interviews().some(i => 
        this.formatDate(new Date(i.scheduledAt)) === dateStr
      );
