...
export class CandidateService {
  ...
  uploadResume(file: File, source: string, jobId?: string | null, applyToJob?: boolean): Observable<Candidate> {
    const formData = new FormData();
    formData.append('file', file);
    if (source) {
    ...
    if (applyToJob) {
      formData.append('applyToJob', String(applyToJob));
    }
  ...
  updateStatus(id: string | number, status: string): Observable<Candidate> {
    return this.api.put<Candidate>(`${this.BASE_URL}/${id}/status`, { status });
  }
